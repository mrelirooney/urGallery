    ...
    <div className="flex justify-end align-end gap-x-6">
      <div className="flex justify-center align-center gap-x-2">
        {Array.from({ length: totalPages }).map((_, idx) => (
          <button className={`w-10 h-10 bg-[var(--light-brown)] text-sm rounded-sm border border-neutral-700 ${idx === (currentPage-1)  ? "bg-neutral-100 text-neutral-900" : "text-neutral-300"}`}
          onClick={() => onChangePage(idx)} key={idx}>
            {idx + 1}
          </button>
        ))}
      </div>
      <button className="p-2 rounded-sm hover:bg-[var(--light-brown)]" onClick={goPrev} aria-label="Previous page">
        <ChevronLeft size={24} />
      </button>
      <button className="p-2 rounded-sm hover:bg-[var(--light-brown)]" onClick={goNext} aria-label="Next page">
